...
|---|---|---:|---|
| A | WebGoat Setup | Teilweise belegt | WebGoat auf EC2 erreichbar; geforderte Setup-Screenshots fehlen im aktuellen Ordner |
| B | SQL Injection | Teilweise belegt | SQL Injection intro abgeschlossen; geforderte B-Screenshots fehlen im aktuellen Ordner |
...
## Abgleich mit dem offiziellen Auftrag

| Teil | Offiziell verlangt | Stand in diesem Ordner |
|---|---|---|
| A | WebGoat-Startseite mit EC2-IP und Security Group Port 8080 | Technisch beschrieben, aber diese zwei Screenshots sind im aktuellen Ordner nicht eindeutig vorhanden |
| B | Login Bypass und Query Chaining mit Payload/Erfolg als Screenshot | SQL-Injection fachlich dokumentiert; offizielle Screenshots fehlen als Bilddateien |
| C | Reflected, DOM-based und Stored XSS mit Screenshots und Antworten | Erfuellt; Screenshots und Antworten vorhanden |
| D | CSRF-Angriffsdatei/Request, Erfolgsscreenshot und Antworten | Erfuellt; Angriffsschema, Live-Status und Antworten vorhanden |
| E | Fremdes Profil lesen/aendern, Screenshot oder Command, Antworten | Erfuellt; Profil-ID, PUT-Command/Response und Antworten vorhanden |
| F | jwt.io, manipulierter Token, Erfolgsscreenshot und Antworten | Erfuellt; Token, jwt.io-Screenshot, Response und Antworten vorhanden |

> [!important]
> Fuer eine perfekte KN02-Abgabe sollten A- und B-Screenshots noch nachgereicht werden. Die fachlichen Inhalte sind vorhanden, aber der Auftrag verlangt dort explizit Bildnachweise.
...
|---|---|
| A | WebGoat-Startseite mit sichtbarer EC2-IP nachreichen |
| A | AWS Security Group Port 8080 mit `My IP` nachreichen |
| B | Login-Bypass-Screenshot mit Payload und gruener WebGoat-Bestaetigung nachreichen |
| B | Query-Chaining-/Integrity-Screenshot mit Payload und Ergebnis nachreichen |
| GitLab Push | GitLab-VAJ-Push ist vorbereitet, braucht aber noch GitLab-Authentifizierung |

> [!todo]
> GitHub ist laut Handoff gepusht. GitLab-Push ist lokal vorbereitet, scheitert aktuell aber an fehlender GitLab-HTTPS/SSH-Authentifizierung auf der EC2-Instanz.
...
- [ ] Setup-Screenshots A nachreichen
- [ ] SQL-Screenshots B nachreichen
